...
class RegisterLogin extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    e.preventDefault();
    if (this.props.login) {
      axios.post('http://localhost:4200/login', this.state.user)
        .then(res => {
    ...
    axios.post('http://localhost:4200/register', this.state.user)
      .then(res => {
  ...
  return (
    ...
    <input type="password" name="password" value={this.state.user.password}placeholder="password" onChange={this.handleChange} />
      {!this.props.login && <input type="text" name="department" placeholder="department" value={this.state.department} onChange={this.handleChange} />}
    <input type="submit" />
    </Form>
